...
/**
 * IMPORTED FROM st.ata.util.FPGenerator in Heretix codebase
 * 
 * <p>
 ...
 * represented as the set of polynomials of degree <i>d</i> with coefficients in
 * <i>Z(2)</i>, modulo an irreducible polynomial <i>P</i> of degree <i>d</i>.
 ...
 * 
 * <p>
 * Let S be a string of bytes and <i>g(S)</i> the string FPGeneratorobtained by
 * taking the byte <code>0x01</code> followed by eight <code>0x00</code> bytes
 * followed by <code>S</code>. Let <i>f(S)</i> be the polynomial associated to
 * the string <i>S</i> viewed as a polynomial with coefficients in the field
 ...
 * stored in the <code>d</code> <strong>most</code> significant bits of a long
 * word.
 * 
 * <p>
